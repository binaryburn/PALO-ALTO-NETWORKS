...
const alertEmail = process.env['alertEmail'];
...
exports.handler = function(event, context, callback) {
    ...
    async.waterfall([
        ...
        function saveConfigToS3(paloConfig, password, callback){
            config.saveConfigToS3(paloConfig, s3Bucket, palo1PrivateIp, function (err, saveActiveResponse) {
                if(err){
                    console.log(err, err.stack);
    ...
    async.waterfall([
        ...
        function setPassivePaloId(paloConfig, password, callback){
            let paloId = 1;
            if (paloConfig.includes(palo1PrivateIp)) paloId = 2;
        ...
        function getManifest(paloConfig, password, fileName, callback) {
            aws.getObjectS3(s3Bucket, fileName, function (err, response) {
                if (err) {
                    console.log(err, err.stack);
        ...
        if (err) {
            config.notifyProcessError(alertEmail, snsTopic, clientName, err + '\n' + results);
